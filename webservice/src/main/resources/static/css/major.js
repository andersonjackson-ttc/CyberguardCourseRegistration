...
function getCourses() {
  $.ajax({method: "GET", url: "/courses?major=1+", dataType: 'json'})
    .done(function(response)  {
      ...
      {
        console.log(9);
        appenddata1+= "<tr><td id='boxCourse'><input type='checkbox' class='taking' value='"+response[j].course_ID+"'></td>" + "<td id='textCourse'>" + response[j].course_ID+ "</td>" + "<td id='textCourse'>"+ response[j].course_Name + "</td></tr>";
      }
      appenddata1+="</table>"
      table.append(appenddata1);
